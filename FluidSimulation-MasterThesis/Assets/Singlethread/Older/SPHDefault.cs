...
public class SPHDefault : MonoBehaviour
{
    ...
    private void ComputeDensityPressure()
    {
        ...
        for (int i = 0; i < amount; i++)
        {
            ...
            for (int j = 0; j < amount; j++)
            {
                ...
                {
                    // Debug.Log("i:" + i + "j:" + j);
                    //Debug.Log("Densidade: " + parameters[particles[i].parameterID].particleMass * (315.0f / (64.0f * Mathf.PI * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadius, 9.0f))) * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadiusSq - r2, 3.0f));
                    particles[i].density += parameters[particles[i].parameterID].particleMass * (315.0f / (64.0f * Mathf.PI * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadius, 9.0f))) * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadiusSq - r2, 3.0f);
                   // particles[j].density += particles[i].density;
                }
            }
            //Debug.Log("Pressure: " + GAS_CONST * (particles[i].density - parameters[particles[i].parameterID].restDensity));
            particles[i].pressure = GAS_CONST * (particles[i].density - parameters[particles[i].parameterID].restDensity);
        }
    }
    ...
    private void ComputeForces()
    {
        for (int i = 0; i < /*countReadyParticles*/ particles.Length; i++)
        {
            ...
            for (int j = 0; j < /*countReadyParticles*/ particles.Length; j++)
            {
                ...
                {
                   // Debug.Log("i:" + i + "j:" + j);
                   // Debug.Log("Pressure: " + -rij.normalized * parameters[particles[i].parameterID].particleMass * (particles[i].pressure + particles[j].pressure) / (2.0f * particles[j].density) * (-45.0f / (Mathf.PI * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadius, 6.0f))) * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadius - r, 2.0f));
                   ...
                   // Debug.Log("Viscosity: " + parameters[particles[i].parameterID].particleViscosity * parameters[particles[i].parameterID].particleMass * (particles[j].velocity - particles[i].velocity) / particles[j].density * (45.0f / (Mathf.PI * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadius, 6.0f))) * (parameters[particles[i].parameterID].smoothingRadius - r));
