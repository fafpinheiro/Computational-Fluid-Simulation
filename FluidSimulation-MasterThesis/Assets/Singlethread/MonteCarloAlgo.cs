...
public class MonteCarloAlgo : MonoBehaviour, Interface
{
    // Consts
    ...
    public void ComputeDensityPressure()
    {
     ...
     //   Dictionary<string, int> temp = new Dictionary<string, int>();
        ...
        for (int i = 0; i < countReadyParticles; i++)
        {
            List<int> particleInteractions = new List<int>();
           ...
           // if (!currentParticles.ContainsKey(i) && particles[i].density != 0)
            ...
            currentParticles.Add(i, particleInteractions);
        }
    ...
    public void ComputeForces()
    {
        ...
        for (int i = 0; i < countReadyParticles; i++)
        {
            ...
            /// {
            currentParticles.TryGetValue(i,out listOfJs);
            foreach (int j in listOfJs)
            {
                if (i == j) continue;
                ...
                if (r < smoothingRadius)// r -> r2
                {
                    ...
                    forceViscosity = viscosityForce(i, j, r);
                     ...
                     dupV[j] += (-forcePressure + (forceViscosity));
