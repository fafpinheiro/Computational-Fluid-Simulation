...
public class A : MonoBehaviour, Interface
{
    // Consts
...
//#pragma warning disable 0649 // This line removes the warning saying that the variable is never assigned to. You can't assign a variable in a struct...
    ...
    private void otimizado()
    {
        ...
        for (int i = 0; i < countReadyParticles; i++)
        {
            ...
            for (int j = i; j < countReadyParticles; j++)
            {
              ...
              //  {
               ...
               // particles[j].density += particleDensity(j, 0);
                ...
                // if (j == countReadyParticles - 1)
                //  {
                //   print(i + " j " + j);
                ///  print("bacon" + i+ " "+ j);

                particles[j].go.transform.position = particles[j].position;
                distBetweenParticles(i, j, out rij, out r2);
                if (i == j) continue;
             ...
             SmoothingKernels.GradientSpiky(rij, smoothingRadius, r)));
                         print("pressure j: " + pressureForce(i, j, rij, r) + " com dividir " + (pressureForce(i, j, rij, r) / (2.0f * particles[j].density)) + " e que tal: " + (-particleMass * (particles[j].pressure + particles[i].pressure) / (2.0f * particles[i].density) *
             SmoothingKernels.GradientSpiky(rij, smoothingRadius, r)));
                     ...
                     dup[i] += (pressureForce(i, j, rij, r) / (2.0f * particles[j].density));
                     dup[j] -= (pressureForce(i, j, rij, r) / (2.0f * particles[i].density));
                     dupV[i] += (viscosityForce(i, j, r) / particles[j].density);
                     dupV[j] -= (viscosityForce(i, j, r) / particles[i].density);*/
                    dup[i] += (pressureForce(i, j, rij, r) /*/ (2.0f * particles[j].density)*/);
                    dupV[i] += (viscosityForce(i, j, r)/* / particles[j].density*/);
                    dup[j] -= (pressureForce(j, i, rij, r) /*/ (2.0f * particles[i].density)*/);
            ...
            particles[i].go.transform.position = particles[i].position;
            particles[i].forcePhysic = dup[i] + dupV[i] + GRAVITY * currenteDesity[i] * gravityMult;
        }
    }
        ...
        for (int i = 0; i < countReadyParticles; i++)
        {
            ...
            for (int j = i; j < countReadyParticles; j++)
            {
                ...
                if (r2 < smoothingRadiusSq)
                {
                    ...
                    particles[i].density += tempDensity;
                    currenteDesity[i] += tempDensity;
                    particles[i].pressure = GAS_CONST * (currenteDesity[i] - restDensity);
                    if (i != j)
                    {
                        currenteDesity[j] += tempDensity;
                        particles[j].density += tempDensity;
                        particles[j].pressure = GAS_CONST * (currenteDesity[j] - restDensity);
                    }
                  ...
                  //  dupV[j] += (viscosityForce(j, i, r) /*/ particles[i].density*/);
                    particles[i].forcePhysic += pressureForce(i, j, rij, r) + viscosityForce(i, j, r) + gravityForce(i);//+ dupV[i]
                    particles[j].forcePhysic += (-pressureForce(j, i, rij, r)) + viscosityForce(j, i, r) + gravityForce(j);//+ dupV[i]
                    particles[j].go.transform.position = particles[j].position;
                }
            }
            particles[i].go.transform.position = particles[i].position;
    ...
    public void ComputeForces()
    {
        ...
        for (int i = 0; i < countReadyParticles; i++)
        {
            ...
            SmoothingKernels.GradientSpiky(rij, smoothingRadius, r)));
                    ...
                    //dupP[i] += (pressureForce(i, j, rij, r) + viscosityForce(i, j, r));
                    //dupP[j] += (-pressureForce(j, i, rij, r) + (viscosityForce(j, i, r)));
                }
